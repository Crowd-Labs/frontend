import Tool, {getMousePos, getMousePosOriginal, getTouchPos, getTouchPosOriginal, hexToRgb, Point} from "./tool";
...
class ColorFill extends Tool {
    private operateStart(pos: Point) {
        ...
        if (colorRgb)
            efficentFloodFill(Tool.ctx, pos.x, pos.y, [colorRgb.r, colorRgb.g, colorRgb.b]);
    }

    public onMouseDown(event: MouseEvent): void {
        event.preventDefault();
        const mousepos = getMousePosOriginal(Tool.ctx.canvas, event);
    ...
    public onTouchStart(event: TouchEvent): void {
        ...
        const touchpos = getTouchPosOriginal(event.target as HTMLCanvasElement, event);
