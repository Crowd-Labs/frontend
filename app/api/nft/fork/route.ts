...
export const POST = async (request) => {
    const { nftName, belongToCollectionId, collectionAddress, nftCreator, forkFrom, imageUrl } = await request.json();

    try {
        await connectToDB();
        const tokenId = await ForkNFT.count({belongToCollectionId})
        const newForkNft = new ForkNFT({ nftName, belongToCollectionId, collectionAddress, nftCreator, forkFrom, imageUrl, tokenId});
