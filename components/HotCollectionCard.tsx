...
function HotCollectionCard(collectionInfo: CollectionInfo) {
  return (
    <Link href={`/collection/${collectionInfo.derivedCollectionAddr}`} className='relative block group rounded-sm overflow-hidden xl:h-[208px] aspect-square'>
      <Image src={collectionInfo.detailJson?.image} alt={collectionInfo.name} className="w-full h-full opacity-90 image-rendering-pixelated" />
